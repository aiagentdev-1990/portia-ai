...
function calculateWeightedAverageAPY(positions) {
  const totalBalance = positions.reduce((sum, p) => sum + BigInt(p.balance), 0n);
  if (totalBalance === 0n)
    return "0";
  const weightedSum = positions.reduce((sum, p) => sum + BigInt(p.apy) * BigInt(p.balance), 0n);
  return (weightedSum / totalBalance).toString();
}
function formatAPY(rawAPY) {
...
function formatUSDC(rawBalance) {
  const value2 = rawBalance / 1e6;
  const formatted = value2.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
var getSupplyAPY = (runtime2, vaultName, vaultAddress) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.evms[0].chainName,
    isTestnet: false
  });
  ...
  if (vaultName === "aave") {
    ...
    runtime2.log(`[getSupplyAPY] Aave: ${apyPercent.toFixed(4)}%`);
    return rawRate;
  } else {
    const eulerUtilLens = runtime2.config.evms[2];
    const callData = encodeFunctionData({
    ...
    const contractCall = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: eulerUtilLens.storageAddress,
        data: callData
      }),
      blockNumber: LAST_FINALIZED_BLOCK_NUMBER
    }).result();
    ...
    runtime2.log(`[getSupplyAPY] ${vaultName}: ${apyPercent.toFixed(4)}%`);
    return normalizedAPY;
  }
};
var getUserBalance = (runtime2, vaultName, vaultAddress, walletAddress, aTokenAddress) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.evms[0].chainName,
    isTestnet: false
  ...
  if (vaultName === "aave") {
    ...
    const sharesCall = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: vaultAddress,
        data: sharesCallData
...
var getATokenAddress = (runtime2, vaultAddress) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.evms[0].chainName,
    isTestnet: false
...
var getUserPreferences = (nodeRuntime) => {
  ...
  const preferences = JSON.parse(new TextDecoder().decode(resp.body));
  return JSON.stringify(preferences);
};
var makeNotifyUser = (userId, message) => (nodeRuntime) => {
  const httpClient = new ClientCapability2;
  const req = {
...
var makeBatchExecute = (operations) => (nodeRuntime) => {
  const httpClient = new ClientCapability2;
  const req = {
...
var makeResearchQuery = (userId, query) => (nodeRuntime) => {
  const httpClient = new ClientCapability2;
  const req = {
...
var makeRebalanceRequest = (request) => (nodeRuntime) => {
  const httpClient = new ClientCapability2;
  const req = {
...
var callResearch = (runtime2, userId, query) => {
  runtime2.log(`[callResearch] Triggering research for ${userId}`);
  runtime2.runInNodeMode(makeResearchQuery(userId, query), consensusIdenticalAggregation())().result();
  runtime2.log(`[callResearch] Research complete`);
};
function validateSchema(moves) {
  if (!Array.isArray(moves))
    return false;
  for (const m of moves) {
    if (typeof m !== "object" || m === null)
      return false;
    if (typeof m.fromVault !== "string" || typeof m.fromAddress !== "string")
      return false;
    if (typeof m.toVault !== "string" || typeof m.toAddress !== "string")
      return false;
    if (typeof m.amount !== "number" || !Number.isInteger(m.amount) || m.amount <= 0)
      return false;
  }
  return true;
}
function validateVaultWhitelist(moves, vaults, vaultAddresses) {
  for (const m of moves) {
    const fromIdx = vaults.indexOf(m.fromVault);
    if (fromIdx === -1 || vaultAddresses[fromIdx].toLowerCase() !== m.fromAddress.toLowerCase())
      return false;
    const toIdx = vaults.indexOf(m.toVault);
    if (toIdx === -1 || vaultAddresses[toIdx].toLowerCase() !== m.toAddress.toLowerCase())
      return false;
  }
  return true;
}
function validateBalanceSum(moves, positions) {
  const withdrawals = {};
  for (const m of moves) {
  ...
  for (const [vaultName, totalWithdrawn] of Object.entries(withdrawals)) {
    ...
    if (!pos || totalWithdrawn > pos.balance)
      return false;
  }
  return true;
}
function validateTargetAPY(moves, positions, targetAPY, log) {
  const balances = {};
  const apys = {};
...
var executeRebalance = (runtime2, moves) => {
  runtime2.log(`[executeRebalance] Starting rebalance with ${moves.length} moves`);
  const operations = [];
  for (const move of moves) {
    const humanAmount = move.amount / 1e6;
  ...
  runtime2.log(`[executeRebalance] Sending ${operations.length} operations in single batch`);
  const batchResponse = runtime2.runInNodeMode(makeBatchExecute(operations), consensusIdenticalAggregation())().result();
  runtime2.log(`[executeRebalance] Batch result: ${batchResponse}`);
  const results = JSON.parse(batchResponse);
  const txRecords = [];
  let resultIdx = 0;
  for (const move of moves) {
  ...
  runtime2.log(`[executeRebalance] All ${moves.length} moves completed`);
  return txRecords;
};
function extractTxHash(responseText) {
  const match = responseText.match(/TX Hash:\s*(0x[0-9a-fA-F]+)/);
  return match ? match[1] : "unknown";
}
var notifyUser = (runtime2, userId, message) => {
  runtime2.log(`[notifyUser] Sending notification to ${userId}`);
  runtime2.runInNodeMode(makeNotifyUser(userId, message), consensusIdenticalAggregation())().result();
  runtime2.log(`[notifyUser] Notification sent`);
};
var onCronTrigger = (runtime2) => {
  runtime2.log("Workflow triggered.");
  const result = runtime2.runInNodeMode(getUserPreferences, consensusIdenticalAggregation())().result();
  const preferences = JSON.parse(result);
  runtime2.log(`Loaded ${preferences.length} user(s)`);
  const apyByVault = {};
  let aaveATokenAddress;
  for (const user of preferences) {
    runtime2.log(`[${user.userId}] Processing user with ${user.vaultAddresses.length} vaults`);
    const positions = [];
    for (let i2 = 0;i2 < user.vaultAddresses.length; i2++) {
      const vaultAddress = user.vaultAddresses[i2];
      const vaultName = user.vaults[i2];
      if (!(vaultAddress in apyByVault)) {
        const rawAPY = getSupplyAPY(runtime2, vaultName, vaultAddress);
        apyByVault[vaultAddress] = rawAPY.toString();
      }
      if (vaultName === "aave" && !aaveATokenAddress) {
        aaveATokenAddress = getATokenAddress(runtime2, vaultAddress);
      }
      const rawBalance = getUserBalance(runtime2, vaultName, vaultAddress, user.safeAddress, vaultName === "aave" ? aaveATokenAddress : undefined);
      ...
      runtime2.log(`[${user.userId}] ${vaultName}: APY=${apyByVault[vaultAddress]} (${formatAPY(apyByVault[vaultAddress])}), balance=${balance} (${formatUSDC(balance)})`);
    }
    const weightedAvgAPY = calculateWeightedAverageAPY(positions);
    const targetAPY = BigInt(Math.round(user.targetPercentageYield * 10000000000000000)).toString();
    const totalBalance = positions.reduce((sum, p) => sum + p.balance, 0);
    runtime2.log(`[${user.userId}] Weighted avg APY: ${weightedAvgAPY} (${formatAPY(weightedAvgAPY)}), target: ${targetAPY} (${formatAPY(targetAPY)})`);
    if (BigInt(weightedAvgAPY) < BigInt(targetAPY)) {
      runtime2.log(`[${user.userId}] Below target, triggering research then rebalance`);
      const positionsSummary = positions.map((p) => `${p.vaultName}: ${formatAPY(p.apy)} APY, ${formatUSDC(p.balance)}`).join("; ");
      const researchQuery = `My DeFi portfolio weighted average APY dropped to ${formatAPY(weightedAvgAPY)} which is below my target of ${formatAPY(targetAPY)}. ` + `Current positions: ${positionsSummary}. ` + `What are the latest USDC yield rates across Aave and Euler on Ethereum mainnet? Are there any recent changes or risks I should know about?`;
      callResearch(runtime2, user.userId, researchQuery);
      const rebalanceReq = {
        userId: user.userId,
        totalBalance,
      ...
      for (let attempt = 1;attempt <= 3; attempt++) {
        runtime2.log(`[${user.userId}] Rebalance attempt ${attempt}/3`);
        try {
          const rebalanceResponse = runtime2.runInNodeMode(makeRebalanceRequest(rebalanceReq), consensusIdenticalAggregation())().result();
          runtime2.log(`[${user.userId}] Attempt ${attempt}: Raw response: ${rebalanceResponse.substring(0, 500)}`);
          const parsed = JSON.parse(rebalanceResponse);
          const moves = parsed.moves ?? parsed;
          runtime2.log(`[${user.userId}] Attempt ${attempt}: Parsed ${Array.isArray(moves) ? moves.length : "non-array"} moves: ${JSON.stringify(moves).substring(0, 500)}`);
          if (!validateSchema(moves)) {
            runtime2.log(`[${user.userId}] Attempt ${attempt}: Schema validation failed`);
            continue;
          }
          if (moves.length === 0) {
            runtime2.log(`[${user.userId}] LLM returned empty moves array`);
            break;
          }
          if (!validateVaultWhitelist(moves, user.vaults, user.vaultAddresses)) {
            runtime2.log(`[${user.userId}] Attempt ${attempt}: Vault whitelist validation failed`);
            continue;
          }
          if (!validateBalanceSum(moves, positions)) {
            runtime2.log(`[${user.userId}] Attempt ${attempt}: Balance sum validation failed`);
            continue;
          }
          if (!validateTargetAPY(moves, positions, targetAPY, (msg) => runtime2.log(`[${user.userId}] Attempt ${attempt}: ${msg}`))) {
            runtime2.log(`[${user.userId}] Attempt ${attempt}: Target APY validation failed`);
            continue;
          }
          validMoves = moves;
          runtime2.log(`[${user.userId}] All validations passed on attempt ${attempt}`);
          break;
        } catch (e) {
          runtime2.log(`[${user.userId}] Attempt ${attempt}: Error - ${String(e)}`);
          continue;
        }
      }
      if (validMoves && validMoves.length > 0) {
        const txRecords = executeRebalance(runtime2, validMoves);
        const txSummary = txRecords.map((tx) => {
        ...
        notifyUser(runtime2, user.userId, `Rebalanced your portfolio. ${txSummary}`);
      } else if (validMoves === null) {
        runtime2.log(`[${user.userId}] All rebalance attempts failed, skipping this cycle`);
      }
    } else {
      runtime2.log(`[${user.userId}] Portfolio APY (${formatAPY(weightedAvgAPY)}) is on target (${formatAPY(targetAPY)})`);
    }
  }
  return {
    result
